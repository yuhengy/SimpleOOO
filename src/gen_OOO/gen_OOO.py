...
def gen_OOO(
  ...
  v += "      for (i=0; i<`ROB_SIZE; i=i+1) begin\n"
  if VERSION==1:
    v += "        if (ROB_state[i]==`READY) begin\n"
    v += "          ROB_rd_data[i] <= ROB_rd_data_wire[i];\n"
    v += "          ROB_taken  [i] <= ROB_taken_wire[i];\n"
    v += "          ROB_next_pc[i] <= ROB_next_pc_wire[i];\n"
  else:
  ...
  if VERSION==1:
    ...
    v += "  reg [`REG_LEN-1:0] memd [`MEMD_SIZE-1:0];\n"
    v += "  always @(posedge clk) begin\n"
    v += "    if (rst) begin\n"
    ...
    v += "  wire [`MEMD_SIZE_LOG-1:0] mem_addr        [`ROB_SIZE-1:0];\n"
    v += "  wire [`REG_LEN-1      :0] ROB_rd_data_wire[`ROB_SIZE-1:0];\n"
    v += "  wire [`ROB_SIZE-1     :0] ROB_taken_wire;\n"
    v += "  wire [`MEMI_SIZE_LOG-1:0] ROB_next_pc_wire[`ROB_SIZE-1:0];\n"
    v += "  generate for (k=0; k <`ROB_SIZE; k=k+1) begin\n"
    v += "  execute execute_instance(\n"
    v += "    .pc(ROB_pc[k]),\n"
    v += "    .op(ROB_op[k]),\n"
    v += "\n"
    v += "    .rs1_imm(ROB_rs1_imm[k]),\n"
    v += "    .rs1_br_offset(ROB_rs1_br_offset[k]),\n"
    v += "    .rs1_data(ROB_rs1_data[k]),\n"
    v += "\n"
    v += "    .rs2_data(ROB_rs2_data[k]),\n"
    v += "\n"
    v += "    .mem_addr(mem_addr[k]),\n"
    v += "    .mem_data(memd[mem_addr[k]]),\n"
    v += "\n"
    v += "    .rd_data_use_alu(ROB_rd_data_use_alu[k]),\n"
    v += "    .rd_data(ROB_rd_data_wire[k]),\n"
    v += "\n"
    v += "    .is_br(ROB_is_br[k]),\n"
    v += "    .taken(ROB_taken_wire[k]),\n"
    v += "    .next_pc(ROB_next_pc_wire[k])\n"
    v += "  );\n"
    v += "  end endgenerate\n"
